/*
 * Copyright (c) 2026 CNCKitchen (Stefan Hermann) and contributors
 * SPDX-License-Identifier: AGPL-3.0-only
 */

export default {
  "theme.dark": "深色主题",
  "theme.light": "浅色主题",
  "theme.toggleTitle": "切换浅色/深色模式",
  "theme.toggleAriaLabel": "切换浅色/深色模式",
  "dropHint.text": "将 <strong>.stl</strong>、<strong>.obj</strong>、<strong>.3mf</strong> 或 <strong>.step</strong> 文件拖放到此处<br/>或<label for=\"stl-file-input\" class=\"link-label\">点击浏览</label>",
  "step.dialogTitle": "STEP 导入",
  "step.modelSize": "模型尺寸：{d} mm",
  "step.custom": "自定义",
  "step.surfaceDeviation": "表面偏差 (mm)",
  "step.normalDeviation": "法线偏差 (°)",
  "step.maxEdge": "最大边长 (mm)",
  "step.import": "导入",
  "step.cancel": "取消",
  "step.coarse": "粗糙",
  "step.standard": "标准",
  "step.fine": "精细",
  "progress.stepParse": "正在读取 STEP 文件…",
  "progress.stepTessellate": "正在剖分 STEP 模型…",
  "ui.wireframe": "线框显示",
  "ui.perspective": "透视视图",
  "ui.controlsHint": "左键拖动：旋转  ·  右键拖动：平移  ·  滚轮：缩放",
  "ui.meshInfo": "{n} 个三角形 · {mb} MB · {sx} × {sy} × {sz} mm",
  "ui.modelScale": "缩放:",
  "ui.modelScaleTitle": "模型缩放 (%)",
  "ui.uniformScaleOn": "等比缩放 (锁定)",
  "ui.uniformScaleOff": "自由缩放 (解锁)",
  "ui.scaleReset": "重置为 100%",
  "ui.fitCamera": "视角居中适应",
  "ui.presetModels": "预设模型",
  "ui.presetCube": "立方体",
  "ui.presetCylinder": "圆柱体",
  "ui.presetBowl": "碗",
  "ui.modeStandard": "标准",
  "ui.modeAutoFit": "自动适配 (图像)",
  "ui.shapeNgon": "正n边形柱",
  "ui.autofitHeight": "高度 (mm)",
  "ui.autofitRepeat": "拼贴数量",
  "ui.autofitSides": "边数 (n ≧ 3)",
  "ui.uploadShort": "上传",
  "ui.autofitStats": "周长: {c} mm · 直径: {d} mm · 单面宽: {w} mm",
  "ui.loadStl": "加载模型…",
  "ui.localProcessingNote": "所有处理都在您的浏览器中本地运行 — 不上传任何数据。",
  "sections.displacementMap": "位移贴图",
  "ui.uploadCustomMap": "上传自定义贴图",
  "ui.noMapSelected": "未选择贴图",
  "ui.customMap": "自定义贴图",
  "ui.removeCustomMap": "移除自定义贴图",
  "ui.loadingTextures": "正在加载纹理…",
  "sections.projection": "投影",
  "labels.mode": "模式",
  "projection.triplanar": "三平面投影",
  "projection.cubic": "立方体（盒子）",
  "projection.cylindrical": "圆柱形",
  "projection.spherical": "球形",
  "projection.planarXY": "平面 XY",
  "projection.planarXZ": "平面 XZ",
  "projection.planarYZ": "平面 YZ",
  "sections.transform": "变换",
  "labels.scaleU": "缩放 U",
  "labels.scaleV": "缩放 V",
  "labels.scaleManualHint": "在输入框中键入数值可获得更小或更大的尺寸",
  "labels.offsetU": "偏移 U",
  "labels.offsetV": "偏移 V",
  "labels.rotation": "旋转",
  "tooltips.proportionalScaling": "等比例缩放 (U = V)",
  "tooltips.proportionalScalingAria": "等比例缩放 (U = V)",
  "sections.displacement": "纹理深度",
  "labels.textureHeight": "纹理高度 (mm)",
  "labels.invertDisplacement": "反向（内推而非外推）",
  "labels.seamBlend": "接缝混合 ⓘ",
  "tooltips.seamBlend": "柔化投影面交接处的硬接缝。对立方体和圆柱形模式有效。",
  "labels.transitionSmoothing": "过渡平滑 ⓘ",
  "tooltips.transitionSmoothing": "接缝边缘附近混合区域的宽度。较低的值使过渡紧贴接缝；较高的值混合更宽的区域。",
  "labels.textureSmoothing": "纹理平滑 ⓘ",
  "tooltips.textureSmoothing": "对位移贴图应用高斯模糊。较高的值会产生更柔和、更渐进的表面细节。0 = 关闭。",
  "labels.capAngle": "封盖角度 ⓘ",
  "tooltips.capAngle": "与垂直方向的夹角（度），决定顶部/底部盖投影的生效范围。较小的值会将盖投影限制在接近平坦的面上。",
  "sections.masking": "遮罩",
  "sections.maskAngles": "按角度 ⓘ",
  "tooltips.maskAngles": "0° = 无遮罩。与水平面夹角在此角度内的表面不会被纹理化。",
  "labels.bottomFaces": "底面",
  "tooltips.bottomFaces": "在与水平面夹角为此范围内的向下表面上抑制纹理",
  "labels.topFaces": "顶面",
  "tooltips.topFaces": "在与水平面夹角为此范围内的向上表面上抑制纹理",
  "sections.surfaceMasking": "按表面 ⓘ",
  "sections.surfaceSelection": "表面选择",
  "tooltips.surfaceMasking": "遮罩表面，以控制哪些区域接收位移。",
  "tooltips.surfaceSelection": "选定的表面显示为绿色，导出时只有这些表面才会接收位移。",
  "excl.modeExclude": "排除",
  "excl.modeExcludeTitle": "排除模式：绘制的表面将不会接收纹理位移",
  "excl.modeIncludeOnly": "仅包含",
  "excl.modeIncludeOnlyTitle": "仅包含模式：只有绘制的表面会接收纹理位移",
  "excl.toolBrush": "笔刷",
  "excl.toolBrushTitle": "笔刷：绘制要排除的三角形",
  "excl.toolFill": "填充",
  "excl.toolFillTitle": "桶形填充：在阈值角度内泛洪填充表面",
  "excl.shiftHint": "按住 Shift 键以擦除",
  "labels.type": "类型",
  "brushType.single": "单个",
  "brushType.circle": "圆形",
  "labels.size": "大小",
  "labels.maxAngle": "最大角度",
  "tooltips.maxAngle": "填充可跨越的相邻三角形之间的最大二面角",
  "ui.clearAll": "全部清除",
  "excl.initExcluded": "0 个面被遮罩",
  "excl.faceExcluded": "{n} 个面被遮罩",
  "excl.facesExcluded": "{n} 个面被遮罩",
  "excl.faceSelected": "{n} 个面被选中",
  "excl.facesSelected": "{n} 个面被选中",
  "excl.hintExclude": "被遮罩的表面显示为橙色，导出时不会接收位移。",
  "excl.hintInclude": "选定的表面显示为绿色，导出时只有这些表面才会接收位移。",
  "precision.label": "精度 (Beta) ⓘ",
  "precision.labelTitle": "在后台细分网格，以便笔刷以更精细的粒度进行选择",
  "precision.outdated": "⚠ 已过期",
  "precision.refreshTitle": "重新细分网格以匹配当前笔刷大小",
  "precision.triCount": "{n} △",
  "precision.refining": "正在细化…",
  "precision.warningBody": "估计约 {n} 个三角形。这可能会减慢您的浏览器运行速度。确定继续吗？",
  "labels.boundaryFalloff": "平滑遮罩 ⓘ",
  "tooltips.boundaryFalloff": "在遮罩边界附近逐渐将位移减至零，防止纹理化区域与非纹理化区域交界处的三角形重叠。",
  "sections.smoothMask": "平滑遮罩 ⓘ",
  "labels.falloffDistance": "距离",
  "tooltips.falloffDistance": "过渡区域的宽度（毫米）。0 = 关闭。",
  "labels.falloffCurve": "过渡",
  "tooltips.falloffCurve": "在平滑遮罩距离内从零到完整纹理高度的渐变形状。",
  "falloffCurve.linear": "线性",
  "falloffCurve.linearTitle": "从遮罩边缘到完整高度保持恒定坡度。",
  "falloffCurve.scurve": "S 曲线",
  "falloffCurve.scurveTitle": "起止平缓——过渡两端无可见折痕。",
  "falloffCurve.ease": "缓入",
  "falloffCurve.easeTitle": "在遮罩边缘起始非常平缓，随后更陡地升至完整高度。",
  "labels.symmetricDisplacement": "对称位移 ⓘ",
  "tooltips.symmetricDisplacement": "启用后，50% 灰色 = 无位移；白色向外推，黑色向内推。可保持零件体积大致恒定。",
  "labels.displacementPreview": "3D 预览 ⓘ",
  "tooltips.displacementPreview": "细分网格并实时位移顶点，以便您判断实际深度。在复杂模型上会大量占用 GPU 资源。",
  "ui.placeOnFace": "放置在面上",
  "ui.placeOnFaceTitle": "点击一个面，使其朝下放置在打印平台上",
  "ui.rotate": "旋转",
  "ui.rotateTitle": "手动围绕 X、Y、Z 轴旋转模型",
  "ui.rotateApply": "应用",
  "ui.rotateReset": "重置",
  "progress.subdividingPreview": "正在准备预览…",
  "warnings.textureHeightOverlap": "⚠ 纹理高度超过了最小模型尺寸的 10% — 导出的 STL 中可能会出现几何重叠。",
  "sections.advancedFeatures": "高级 / Beta 功能 ⓘ",
  "tooltips.advancedFeatures": "实验性工具 — 行为可能会随版本变化。",
  "ui.advancedBlurb": "实验性工具。行为可能会随版本变化。",
  "ui.bakeTexturesHeading": "烘焙纹理",
  "ui.bakeTexturesDesc": "将当前纹理应用到网格，并将结果重新加载为工作模型，以便继续编辑。",
  "ui.bakeTextures": "烘焙纹理",
  "tooltips.bakeTextures": "将当前纹理应用到网格，并将结果重新加载为工作模型。",
  "ui.bakeMaskNewFaces": "遮罩刚烘焙的面，防止其被进一步纹理化",
  "ui.noDownwardZHeading": "悬垂保护",
  "ui.noDownwardZDesc": "某些纹理会将表面顶点向下推，产生新的悬垂。启用后，纹理化过程中顶点将永远不会沿 −Z 方向移动。X 和 Y 方向的移动不受影响。",
  "ui.noDownwardZ": "防止向下 (−Z) 的顶点位移",
  "ui.smoothBottomHeading": "平滑底面",
  "ui.smoothBottomDesc": "纹理化后，将距底平面 0.1 mm 以内的顶点吸附到底平面上。保持与打印平台接触的表面完全平整，避免切片软件在底部细小三角形上渲染出微小的高度差。",
  "ui.smoothBottom": "将接近底部的顶点吸附到底平面",
  "ui.harvestFlatHeading": "回收平坦面",
  "ui.harvestFlatDesc": "当简化达到三角形数量限制时，通常仍剩余一些几乎无成本即可移除的平坦面。启用后，只要每次合并使表面偏移小于下方设置的容差，简化就会继续合并这些平坦面 — 在不损失可见细节的情况下进一步节省三角形。",
  "ui.harvestFlat": "在三角形限制以下继续合并平坦面",
  "ui.harvestTol": "平坦容差 (mm)",
  "tooltips.harvestTol": "平坦面合并允许引入的最大表面偏差。数值越高，可在更大偏差下移除更多三角形；约 0.005 mm 对 FDM 打印来说不可察觉。",
  "ui.preserveUntexturedHeading": "保留无纹理表面",
  "ui.preserveUntexturedDesc": "使未应用纹理的表面（涂抹遮罩、角度遮罩）与源模型完全一致：网格规整化和减面不会移动或删除这些表面的任何顶点，圆角和细节得以完整保留。如果仅无纹理表面就超过三角形上限，导出将超出上限，而不是降低纹理质量。",
  "ui.preserveUntextured": "不修改无纹理表面",
  "alerts.bakeFailed": "烘焙失败：{msg}",
  "progress.finalizing": "正在完成…",
  "sections.export": "导出 ⓘ",
  "tooltips.export": "边长越小，位移细节越精细。输出随后会根据三角形数量限制进行简化。",
  "labels.resolution": "分辨率 (mm)",
  "labels.resCoarse": "粗糙",
  "labels.resFine": "精细",
  "tooltips.resolution": "导出时，长度大于此值的边将被分割",
  "warnings.resolutionTooCoarse": "⚠ 分辨率粗于模型包围盒对角线的 1/100 — 精细细节将丢失。请降低该值以获得更精细的结果。",
  "labels.smartRes": "推荐数值（可按需调整）",
  "tooltips.smartRes": "根据当前纹理的细节、模型的表面积和位移幅度自动设置分辨率和最大三角形数。",
  "ui.smartResInfo": "智能：<b>{edge} mm</b> · 最多 <b>{tris}</b> 个三角形 · PPE {ppe} · 像素 {pix} mm · 表面积 {area} cm²",
  "ui.smartResBudgetCapped": "已达内存上限",
  "labels.outputTriangles": "输出三角形数",
  "tooltips.outputTriangles": "网格首先会被完全细分，然后再简化至该数量",
  "warnings.safetyCapHit": "⚠ 细分过程中达到了 2000 万三角形的安全上限 — 结果可能仍比请求的边长要粗糙。",
  "warnings.preserveOverBudget": "⚠ 仅无纹理表面就已超过三角形上限 — 这些表面被保留，因此导出超出上限。",
  "ui.exportStl": "导出 STL",
  "ui.export3mf": "导出 3MF",
  "tooltips.exportStl": "兼容所有切片软件",
  "tooltips.export3mf": "文件体积更小 — 可能不兼容部分切片软件",
  "progress.writing3mf": "正在写入 3MF…",
  "progress.subdividing": "正在细分网格…",
  "progress.refining": "正在优化：{cur} 个三角形，最长边 {edge}",
  "progress.regularizing": "正在规整狭长三角形…",
  "progress.applyingDisplacement": "正在对 {n} 个三角形应用位移…",
  "progress.displacingVertices": "正在位移顶点…",
  "progress.repairingMesh": "正在封闭 T 形接点…",
  "progress.decimatingTo": "正在简化 {from} → {to} 个三角形…",
  "progress.decimating": "正在简化：{cur} → {to} 个三角形",
  "progress.harvestingFlat": "正在移除平坦面…",
  "progress.writingStl": "正在写入 STL…",
  "progress.done": "完成！",
  "progress.processing": "正在处理…",
  "license.btn": "许可证与条款",
  "license.title": "许可证与条款",
  "license.item1": "可免费用于任何目的，包括<strong>商业用途</strong>（例如为客户或产品进行 STL 纹理化）。",
  "license.item2": "按原样使用本工具时，<strong>感谢</strong>您的署名，但<strong>并非必需</strong>。",
  "license.item3": "关于交错分层打印技术（“振り重ね”）的详细说明请参阅作者在 <a href=\"https://note.com/mithril_mex/n/nf6866893448c\" target=\"_blank\" rel=\"noopener\">note</a> 上的文章。<br>欢迎打赏支持！",
  "license.item4": "此工具按<strong>原样</strong>提供，<strong>不提供任何形式的</strong>担保。使用风险自负。",
  "license.item5": "<strong>不提供</strong>技术支持。作者没有义务修复错误、回答问题或更新本工具。",
  "license.item6": "对于因使用本工具而产生的任何损害、数据丢失或问题，作者<strong>不承担任何责任</strong>。",
  "license.item8": "源代码可在 <a href=\"https://github.com/Mithril-3d/BumpMesh_Color\" target=\"_blank\" rel=\"noopener\">GitHub</a> 上获取。",
  "imprint.btn": "隐私政策",
  "imprint.title": "隐私政策",
  "imprint.privacyMeshLocal": "所有 3D 模型（STL、OBJ、3MF、STEP）和纹理图像均在您的浏览器中完全本地处理。任何文件或几何数据均不会上传至任何外部服务器。",
  "imprint.privacyHosting": "本网站托管于 <strong>GitHub Pages</strong>（GitHub Inc. / Microsoft Corp.，美国加利福尼亚州旧金山市 Colin P Kelly Jr St 88 号，邮编 94107）。当您访问本网站时，GitHub 可能会在标准服务器日志中处理您的 IP 地址。请参阅 <a href=\"https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement\" target=\"_blank\" rel=\"noopener\">GitHub 隐私声明</a>。",
  "storage.modeLabel": "自动保存:",
  "storage.session": "会话保存",
  "storage.local": "长期保存",
  "storage.sessionDesc": "关闭标签页前保持设置 (sessionStorage)",
  "storage.localDesc": "在浏览器中永久保持设置 (localStorage)",
  "imprint.privacyLocal": "本工具根据您的选择，将用户设置（语言、主题、纹理设置、映射设置及各项UI偏好）保存在浏览器的 <strong>sessionStorage</strong> 或 <strong>localStorage</strong> 中。这些数据仅保存在您的浏览器内，绝不会传输到任何外部服务器，也不用于Cookie或用户追踪。",
  "imprint.privacyNoCookies": "本网站<strong>不使用</strong> Cookie、分析或任何跟踪技术。",
  "alerts.loadFailed": "无法加载模型：{msg}",
  "alerts.exportFailed": "导出失败：{msg}",
  "alerts.exportOOM": "导出失败：模型在细分过程中内存不足。\n\n当网格在所需的分辨率下需要过多三角形时，会出现此问题。\n\n解决方法：\n\u2022 增大分辨率 (mm) 的数值，以减少生成的三角形数量",
  "alerts.fileTooLarge": "文件过大 ({size} MB)。允许的最大值：{max} MB。",
  "alerts.degenerateTrianglesRemoved": "加载时，已从网格中移除 {n} 个无效三角形（坐标为 NaN 或面积为零）。模型仍可正常使用。",
  "diag.runAdvanced": "运行高级检查",
  "diag.runAdvancedTitle": "检查相交和重叠的三角形（大型网格的检查可能需要一些时间）",
  "diag.running": "正在检查…",
  "diag.meshOk": "\u2714 网格看起来没问题",
  "diag.openEdges": "{n} 条开放边 \u2014 网格非封闭",
  "diag.nonManifoldEdges": "{n} 条非流形边",
  "diag.multipleShells": "{n} 个不连通的外壳",
  "diag.intersectingTris": "{n} 对相交三角形",
  "diag.overlappingTris": "{n} 个重叠/重复三角形",
  "diag.advancedOk": "\u2714 未发现相交或重叠",
  "diag.recommendFix": "进行纹理处理前，请先在您的 CAD 软件、切片软件或<a href=\"https://www.formware.co/onlinestlrepair\" target=\"_blank\" rel=\"noopener\">在线工具</a>中修复这些问题。",
  "diag.show": "显示",
  "diag.hide": "隐藏",
  "header.exportProject": "保存项目 (.bumpmesh)",
  "header.importProject": "加载项目 (.bumpmesh)",
  "header.save": "保存项目",
  "header.load": "加载项目",
  "header.resetSettings": "重置为默认值",
  "header.undo": "撤销 (Ctrl+Z)",
  "header.redo": "重做 (Ctrl+Shift+Z)",
  "alerts.resetConfirm": "是否将所有设置重置为默认值？",
  "header.exportSettingsLabel": "设置",
  "header.exportModelLabel": "模型 (STL)",
  "header.exportTextureLabel": "自定义纹理",
  "header.exportGo": "保存",
  "header.loadDialogTitle": "此项目包含模型。加载：",
  "header.loadModelLabel": "模型 + 设置",
  "header.loadSettingsLabel": "仅设置",
  "header.loadGo": "加载",
  "alerts.importFailed": "无法加载项目：{msg}",
  "labels.snapSeamless": "吸附至无缝环绕 ⓘ",
  "tooltips.snapSeamless": "将 U 轴缩放吸附到整数环绕值，使纹理在圆柱面或球面上无缝环绕。",
  "labels.cylinderAxis": "圆柱轴",
  "ui.cylinderAutofit": "自动拟合",
  "tooltips.cylinderAutofit": "使用最小二乘法将圆形拟合到零件的外向顶点。",
  "ui.cylinderReset": "重置",
  "tooltips.cylinderReset": "将轴重置为包围盒中心，半径重置为包围盒推导值。",
  "ui.cylinderPanelAria": "圆柱轴放置",
  "ui.cylinderPanelLabel": "定义圆柱投影",
  "ui.cylinderNoModel1": "加载模型以定位",
  "ui.cylinderNoModel2": "圆柱轴",
  "ui.cylinderPanelMinimize": "最小化 / 还原",
  "ui.presetModels": "Preset Models",
  "ui.presetCube": "Cube",
  "ui.presetCylinder": "Cylinder",
  "ui.presetBowl": "Bowl",
  "labels.textureSizeU": "Size U",
  "labels.textureSizeV": "Size V",
  "tooltips.textureSize": "Physical size of one texture tile in mm",
  "progress.slicingMesh": "Slicing mesh at layer boundaries…",
  "progress.applyingLayerDisplacement": "Applying layer-aligned displacement…",
  "progress.assigningTools": "Assigning colors to triangles…",
  "progress.weldingMesh": "Welding & deduplicating vertices…",
  "progress.building3mf": "Building 3MF model structure…",
  "progress.packaging3mf": "Compressing 3MF package…",
  "license.forkNotice": "<strong>Fork & Disclaimer:</strong> BumpMesh_Color is an independent modification by <a href=\"https://x.com/Mithril_MEX\" target=\"_blank\" rel=\"noopener\">@Mithril_MEX</a>. CNC Kitchen (Stefan Hermann) is <strong>not affiliated</strong> with this fork and does <strong>not provide support</strong> for it. Please report all issues to @Mithril_MEX.",

  // Color & Multi-Tool Translations
  "color.sectionTitle": "彩色与多挤出机",
  "color.enabled": "启用",
  "color.tabQuantize": "颜色量化",
  "color.tabLayerBlend": "交错层叠",
  "color.show3DPreview": "在3D预览中显示颜色",
  "color.untexturedTool": "非纹理区域工具",
  "color.untexturedToolTitle": "分配给遮罩区域或未纹理化表面（如底面）的工具",
  "color.extruderCount": "挤出机数量 (颜色数)",
  "color.extruderCountTitle": "提取的主色数量（挤出机数量）",
  "color.assignLabel": "分配:",
  "color.interleavedDesc": "按固定层厚交错堆叠各颜色工具；在目标颜色区域凸起，非目标颜色区域凹陷。",
  "color.layerThickness": "层厚 (mm)",
  "color.layerThicknessTitle": "每个工具的单层厚度（与切片软件的层高匹配）",
  "color.convexAmp": "凸起偏移 (mm)",
  "color.convexAmpTitle": "与目标颜色匹配的层的凸起量",
  "color.concaveAmp": "凹陷偏移 (mm)",
  "color.concaveAmpTitle": "与目标颜色不匹配的层的凹进量",
  "color.profileMode": "剖面形状 (遮蔽)",
  "color.profileModeTitle": "每层的截面几何形状。平面台阶可产生平直清晰的边缘（标准 / 推荐）。",
  "color.profileStep": "平面台阶 (标准 / 推荐)",
  "color.profileLouver": "45° 百叶遮檐 (异色遮蔽 / 实验性)",
  "color.shadingMode": "色调表现",
  "color.shadingModeTitle": "色调表现。锐化呈现清晰二值；渐变则依据原图亮度平滑控制凸起高度。",
  "color.shadingSharp": "锐利 (二值 / 0-1)",
  "color.shadingGradient": "渐变 (连续色调)",
  "color.interleavedTools": "交错工具",
  "color.interleavedStructure": "🔄 交错切片结构",
  "color.infoPitch": "• 层高: {pitch} mm (总层数: 约{layers}层)",
  "color.infoConvex": "• 凸起量: +{val} mm (目标颜色匹配)",
  "color.infoConcave": "• 凹进量: -{val} mm (不匹配)",
  "color.infoProfile": "• 剖面: {profile}",
  "color.infoShading": "• 色调: {shading}",
  "color.infoPattern": "• 交错模式: {pattern} → …",
  "color.exportBtn": "🎨 导出多工具 3MF",
  "color.exportBtnTitle": "导出完全兼容 PrusaSlicer、OrcaSlicer 和 Bambu Studio 的多工具 3MF 文件",
  "color.interleavedPreviewMap": "🖼️ 查看交错调制图 (2D 映射)",
  "color.mapModalTitle": "🖼️ 交错调制图 (混合权重图) 预览",
  "color.mapModalModeLabel": "显示模式:",
  "color.mapModalModeCurrent": "① K-means 质心裁剪",
  "color.mapModalModeFullrange": "② 色调保持 (全范围亮度 / 保留暗部)",
  "color.mapModalModeOriginal": "③ 原始纹理图像",
  "color.mapModalGamma": "色调伽马:",
  "color.mapModalLegend": "※ 白 (1.0) = Tool 1 最大突出 / 黑 (0.0) = Tool 2 最大突出 / 中灰 (0.5) = 等宽暴露",
  "color.mapModalSavePng": "💾 保存 PNG 图像",
  "color.mapModalClose": "关闭",
  "color.mapModalHoverHint": "将鼠标悬停在图像上以查看像素详情",
  "color.mapModalLoading": "正在加载调色板信息...",
  "color.mapModalNoTexture": "请先加载纹理图像。",
};
